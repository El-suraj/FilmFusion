...
const WatchlistsPage = () => {
    ...
    const fetchWatchlists = async () => {
        ...
        try {
            ...
            console.error('Error fetching watchlists:', err.response?.data?.message || err.message);
    ...
    const handleDeleteWatchlist = async (watchlistId) => {
        if (window.confirm('Are you sure you want to delete this watchlist?')) {
            try {
                ...
                console.error('Error deleting watchlist:', err.response?.data?.message || err.message);
                alert(`Failed to delete watchlist: ${err.response?.data?.message || 'Server error'}`);
            }
        }
    };

    if (authLoading || loading) {
        return <div className="container">Loading watchlists...</div>;
    }
    ...
    return (
        <div className="container">
            <h2>My Watchlists</h2>
            <button onClick={() => setShowCreateModal(true)} className="btn create-watchlist-btn">
                ...
                <div className="watchlist-list">
                    {watchlists.map((watchlist) => (
                        <div key={watchlist._id} className="watchlist-item">
                            <h3><Link to={`/watchlists/${watchlist._id}`}>{watchlist.name}</Link></h3>
                            <p>{watchlist.description}</p>
                            <p>Movies: {watchlist.movies.length}</p>
                            <button onClick={() => handleDeleteWatchlist(watchlist._id)} className="btn delete-btn">Delete</button>
                        </div>
                    ))}
                </div>
            )}
